...
static void before(hook_fargs6_t *args, void *udata)
{
    ...
    pr_info("before: syscall cmd=0x%lx\n", cmd);

    // 如果命令不在合法范围内则直接返回
    if (cmd < SUPERCALL_HELLO || cmd > SUPERCALL_MAX) {
        pr_info("before: cmd out of range, skip\n");
        return;
    }
    ...
    if (len <= 0) {
        pr_info("before: failed to copy key from user, len=%ld\n", len);
        return;
    }
    pr_info("before: copied key='%s', len=%ld\n", key, len);
    ...
    pr_info("before: current uid=%u, is_trusted_manager=%d\n",
            current_uid(), is_trusted_manager);

    if (is_trusted_manager) {
        is_key_auth = 1;
        pr_info("before: trusted manager, key auth automatically granted\n");
    }

    // 进行 key 授权校验
    if (!auth_superkey(key)) {
        is_key_auth = 1;
        pr_info("before: superkey auth passed\n");
    } else if (!strcmp("su", key)) {
        uid_t uid = current_uid();
        if (!is_su_allow_uid(uid) && !is_trusted_manager) {
            pr_info("before: 'su' key denied for uid=%u\n", uid);
            return;
        }
        pr_info("before: 'su' key allowed for uid=%u\n", uid);
    } else {
        if (!is_trusted_manager) {
            pr_info("before: key '%s' denied, not trusted manager\n", key);
            return;
        }
    }

    // 获取剩余 syscall 参数 a1-a4
    ...
    pr_info("before: calling supercall with args: %lx %lx %lx %lx\n",
            a1, a2, a3, a4);
    ...
    pr_info("before: supercall returned %ld\n", args->ret);
}
